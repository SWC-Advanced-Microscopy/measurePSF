...
% Electrical and dark noise plots
%
% mpsf.plot.electrical_noise(fname_elec,fname_dark)
%
% Purpose
% Plots of electrical noise for each channel with PMTs off and dark noise with PMTs on
...
for ii=1:size(imstack_elec,3)
    ...
    title(sprintf('Channel %d SD_{dark}=%0.2f', ii, std(t_im_dark(:))))
    grid on
    hold off
end
hl = legend('Electrical Noise', 'Dark Noise', 'Orientation','horizontal');
